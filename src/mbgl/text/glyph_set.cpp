...
void align(Shaping &shaping, const float justify, const float horizontalAlign,
           const float verticalAlign, const float maxLineLength, const float lineHeight,
...
void justifyLine(std::vector<PositionedGlyph> &positionedGlyphs,
                 ...
                 uint32_t start, uint32_t end, float justify) {
    if (!justify)
        return;
...
float GlyphSet::determineIdeographicLineWidth(const std::u16string& logicalInput, const float spacing, float maxWidth) const
{
    float totalWidth = 0;
...
std::set<int32_t> GlyphSet::determineLineBreaks(const std::u16string& logicalInput, const float spacing, float maxWidth) const
{
    if (!maxWidth)
        return {};
    ...
    for (uint32_t i = 0; i < logicalInput.size(); i++) {
        ...
        if (util::i18n::allowsWordBreaking(glyph.id) || util::i18n::allowsIdeographicBreaking(glyph.id)) {
            lastSafeBreak = i;
